...
<button
  ...
  type="button" on:click={() => handleAddRow()}>
  新規登録
</button>
...
<div
  ...
  aria-hidden="true"
  bind:this={is_hidden}
  class="hidden overflow-y-auto overflow-x-hidden fixed top-[12vh] left-0 right-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
  ...
  <div class="relative p-4 w-full max-w-md max-h-full">
      <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
          ...
          <div class="p-4 md:p-5 text-center">
              ...
              <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">このレコードを削除しますか？</h3>
              {#if currentRow}
              <div>
                <span class="inline-block">{currentRow.name}</span>
              </div>
              {/if}
              <button
